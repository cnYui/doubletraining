...
- 2026-09-12: merged to `main` (PR #2) and re-imported into the original Studio project (`tree/main/agent`, in place). A `/debug … pages/plan/index` chat card there rendered the **compact layout** ("Sat, Sep 12 · 5 exercises · Chest · Bench Press · 5 × 8 · 80 kg"), so plain-class rules inside `@media (max-height: 240px)` do apply in the card.
- Not verified: "remove" through the model, the double-tap cancel (a simulator double tap never reaches Pages), the 60 s timeout actually firing (the earlier 30 s timeout never fired during a 90 s call — cause unknown), everything on physical glasses (including whether `RECORD_AUDIO` must be declared).
