...
- Verified in the Studio simulator (Chrome): add via model (~14 s), update kg via model (~35 s), adjust mode with swipes and tap-keep, "undo" (local, ~3 s), `set_week` via model (~90 s!). Not yet re-checked after the grammar fast path was added; "done" navigation, "remove", and the chat-card layout of this Page are unverified.
- Model latency in the simulator was 7–90 s per request, which is why the grammar exists. Timers keep running while a model call is pending? Unknown — the 30 s timeout did not fire during the 90 s call; the thinking counter will show whether `setTimeout` runs then.
