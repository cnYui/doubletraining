...
- Both Pages accept an optional `date`, which must be `today`, `tomorrow`, `yesterday`, or a calendar date in `YYYY-MM-DD` form. Convert any other date to `YYYY-MM-DD`. Omit `date` when it is unclear; the Page then uses today.
- Examples: `start today's workout` → day plan `{ "date": "today" }`; `what do I train tomorrow` → day plan `{ "date": "tomorrow" }`; `show my training plan` → calendar `{}`.
- Requests may arrive in any language; always pass the English date values above. Respond in English.
- This version cannot change weights, reps, or plans by voice. For such requests, explain that sets are logged on the Page with the temple touchpad, or that the request is not supported yet; never claim a change was made.
...
- No network, camera, microphone, notifications, Widgets, or Agent Workers.
